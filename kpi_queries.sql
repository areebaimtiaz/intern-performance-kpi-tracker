...
DB_PATH = "interns.db"
KPI_COLS = ["Project Quality", "Mentor Feedback Proxy", "Engagement Score"]
...
def main(period_label: str):
    ...
    df = pd.read_sql(
        'SELECT "Intern ID","Intern Role","Department",'
        '"Project Quality","Mentor Feedback Proxy","Engagement Score" '
        'FROM interns_raw', conn)
    conn.close()

    wb = Workbook()

    # --- Raw Data sheet (with a live Flag formula, not a hardcoded value) ---
    ws_raw = wb.active
    ...
    # --- Summary by Intern Role (formulas reference Raw Data, not hardcoded) ---
    ...
    roles = sorted(df["Intern Role"].unique())
    headers = ["Intern Role", "Avg Project Quality", "Avg Mentor Feedback", "Avg Engagement", "# Interns"]
    ...
    for col, w in zip("ABCDE", [28, 18, 18, 16, 12]):
    ...
    for j, h in enumerate(["Department", "Avg Project Quality", "Avg Mentor Feedback",
                           "Avg Engagement", "# Interns"], start=1):
    ...
    for col, w in zip("ABCDE", [24, 18, 18, 16, 12]):
